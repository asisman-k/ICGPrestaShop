...
*/
	...
	if( $result > 0 ){//Hi ha productes a crear/actualitzar
		foreach ($result->fetchAll(PDO::FETCH_ASSOC) as $msrow_product)
		{
			...
			$result3 = $myDB->consulta("SELECT * FROM icgps.icg_ps_producte WHERE icg_producte = ".$msrow_product['CODARTICULO']." AND icg_color = '".$msrow_product['COLOR']."' AND icg_talla = '".$msrow_product['TALLA']."'");
			...
			if( $myrows_prestashop > 0 ){ //És una actualització de producte
                ...
                if( $msrow_product['DESCATALOGADO'] != $myrow_producte['descatalogado']){
                    ...
                    $myDB->consulta("UPDATE icgps.icg_ps_producte SET descatalogado = '".$msrow_product['DESCATALOGADO']."' , timestamp = '".date("Y-m-d H:i:s")."', flag_actualitzat = 1 WHERE icg_producte = ".$msrow_product['CODARTICULO']." AND icg_color = '".$msrow_product['COLOR']."' AND icg_talla = '".$msrow_product['TALLA']."'");
                ...
                if( intval($msrow_product['CODBARRAS']) != $myrow_producte['ean13']){
                    //S'ha actualitzat el descatalogat
                    $myDB->consulta("UPDATE icgps.icg_ps_producte SET ean13 = '".$msrow_product['CODBARRAS']."' , timestamp = '".date("Y-m-d H:i:s")."', flag_actualitzat = 1 WHERE icg_producte = ".$msrow_product['CODARTICULO']." AND icg_color = '".$msrow_product['COLOR']."' AND icg_talla = '".$msrow_product['TALLA']."'");
                    $eans_actualitzats++;
                ...
                if( $msrow_product['Visible_Web'] != $myrow_producte['visibleweb']){
                    //S'ha actualitzat el descatalogat
                    $myDB->consulta("UPDATE icgps.icg_ps_producte SET visibleweb = '".$msrow_product['Visible_Web']."', timestamp = '".date("Y-m-d H:i:s")."', flag_actualitzat = 1 WHERE icg_producte = ".$msrow_product['CODARTICULO']." AND icg_color = '".$msrow_product['COLOR']."' AND icg_talla = '".$msrow_product['TALLA']."'");
                ...
                $productes_jatrobats++;
					...
					if($msrow_product['Codproveedor'] == 0){
						$result2 = $myDB->consulta("INSERT INTO icgps.icg_ps_producte (icg_producte, icg_talla, icg_color, descripcio, timestamp,flag_actualitzat, ean13, descatalogado, visibleweb) VALUES (".$msrow_product['CODARTICULO'].", '".$utils->encodeToUtf8($msrow_product['TALLA'])."', '".$utils->encodeToUtf8($msrow_product['COLOR'])."','".$utils->encodeToUtf8($msrow_product['DESCRIPCION'])."', '".date("Y-m-d H:i:s")."',1,'".$msrow_product['CODBARRAS']."','".$msrow_product['DESCATALOGADOS']."','".$msrow_product['Visible_Web']."')");
					}else{
						$result2 = $myDB->consulta("INSERT INTO icgps.icg_ps_producte (icg_producte, icg_talla, icg_color, descripcio, timestamp,flag_actualitzat, fabricant, ean13, nom_fabricant) VALUES (".$msrow_product['CODARTICULO'].", '".$utils->encodeToUtf8($msrow_product['TALLA'])."', '".$utils->encodeToUtf8($msrow_product['COLOR'])."','".$utils->encodeToUtf8($msrow_product['DESCRIPCION'])."', '".date("Y-m-d H:i:s")."',1,".$msrow_product['Codigo_Marca'].",'".$msrow_product['CODBARRAS']."','".$utils->encodeToUtf8($msrow_product['Descripcion_Marca'])."')");
					}
					...
					*/
					$result_preu = $msDB->consulta("SELECT * FROM view_imp_preus WHERE Codarticulo = ".$msrow_product['CODARTICULO']." AND Talla = '".$msrow_product['TALLA']."' AND Color = '".$msrow_product['COLOR']."'");
					//Per a cada nou stock
					$msrow_preu = $result_preu->fetch(PDO::FETCH_ASSOC);
					$myDB->consulta("INSERT INTO icgps.icg_ps_preus (tarifa, icg_producte, icg_talla, icg_color, timestamp, pvp, dto_percent, preu_oferta, dto_euros, iva, pvp_siva, preu_oferta_siva, dto_euros_siva, flag_actualitzat) VALUES (1,".$msrow_product['CODARTICULO'].", '".$utils->encodeToUtf8($msrow_product['TALLA'])."', '".$utils->encodeToUtf8($msrow_product['COLOR'])."','".date("Y-m-d H:i:s")."',".$msrow_preu['Pbruto_iva'].",".$msrow_preu['Dto_porc'].",".$msrow_preu['Pneto_iva'].",".$msrow_preu['Dto_impote_iva'].",".$msrow_preu['Iva'].",".$msrow_preu['Pbruto_s_iva'].",".$msrow_preu['Pneto_s_iva'].",".$msrow_preu['Dto_importe_s_iva'].",1)");
					if($myDB->mysql_affected_rows() > 0){
					...
					*/
					$result_stock = $msDB->consulta("SELECT * FROM view_imp_stocks WHERE Codarticulo = ".$msrow_product['CODARTICULO']." AND Talla = '".$msrow_product['TALLA']."' AND Color = '".$msrow_product['COLOR']."'");
					//Per a cada nou stock
					$msrow_stock = $result_stock->fetch(PDO::FETCH_ASSOC);
					$myDB->consulta("INSERT INTO icgps.icg_ps_stocks (icg_producte, icg_talla, icg_color, timestamp,ean13,stock_actual,flag_actualitzat) VALUES (".$msrow_product['CODARTICULO'].", '".$utils->encodeToUtf8($msrow_product['TALLA'])."', '".$utils->encodeToUtf8($msrow_product['COLOR'])."','".date("Y-m-d H:i:s")."','".$msrow_product['CODBARRAS']."',".intval($msrow_stock['Stock_disponible']).",1)");
